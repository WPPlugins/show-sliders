...
define ( 'WEAVER_SLIDER_PI_VERSION','1.4.9');
...
function atw_slider_register_post_cpt() {
    $singular_item = __('Weaver Slider Post', 'show-sliders');
    $plural_item = __('Weaver Slider Posts', 'show-sliders');
    ...
    $labels = array(
        ...
        'add_new'            => __('New', 'show-sliders') . ' ' .  $singular_item,
        'add_new_item'       => __('New', 'show-sliders') . ' ' .  $singular_item,
        'edit_item'          => __('Edit', 'show-sliders') . ' ' .  $singular_item,
        'new_item'           => __('New', 'show-sliders') . ' ' .  $singular_item,
        'all_items'          => __('All', 'show-sliders') . ' ' .  $plural_item,
        'view_item'          => __('View', 'show-sliders') . ' ' .  $singular_item,
        'search_items'       => __('Search', 'show-sliders') . ' ' .  $plural_item,
        'not_found'          => sprintf( __( 'No %s found', 'show-sliders' ), $plural_item ),
        'not_found_in_trash' => sprintf( __( 'No %s found in trash', 'show-sliders' ), $plural_item ),
        'parent_item_colon'  => '',
    ...
    $args = array(
        'labels'             => $labels,
        'description'       =>  __('Auxiliary Custom Post Type to use for defining sliders.', 'show-sliders'),
  ...
  add_post_type_support( 'atw_slider_post', 'excerpt'); // even though listed in the 'supports' list, needs this explicit add...
    ...
    $tlabels = array(
        'name'                       => _x( 'Slider Group', 'Taxonomy General Name', 'show-sliders' ),
        'singular_name'              => _x( 'Slider Group', 'Taxonomy Singular Name', 'show-sliders' ),
        'menu_name'                  => __( 'Slider Groups', 'show-sliders' ),
        'all_items'                  => __( 'All Slider Groups', 'show-sliders' ),
        'parent_item'                => __( 'Parent Slider Group', 'show-sliders' ),
        'parent_item_colon'          => __( 'Parent Slider Group:', 'show-sliders' ),
        'new_item_name'              => __( 'New Slider Group', 'show-sliders' ),
        'add_new_item'               => __( 'Add New Slider Group', 'show-sliders' ),
        'edit_item'                  => __( 'Edit Slider Group', 'show-sliders' ),
        'update_item'                => __( 'Update Slider Group', 'show-sliders' ),
        'separate_items_with_commas' => __( 'Separate Slider Groups with commas', 'show-sliders' ),
        'search_items'               => __( 'Search Slider Groups', 'show-sliders' ),
        'add_or_remove_items'        => __( 'Add or remove Groups', 'show-sliders' ),
        'choose_from_most_used'      => __( 'Choose from the most used groups', 'show-sliders' ),
    );
    $targs = array(
        'labels'                     => $tlabels,
        'hierarchical'               => true,
...
function atw_slider_add_meta_box() {
   ...
   add_meta_box(
       'atw_slider_intro',
       __( 'Weaver Slider Post Introduction', 'show-sliders' ),
...
function atw_slider_enqueue_scripts() {	// enqueue runtime scripts
	...
	// $at_end = !atw_posts_get_slider_opt('scriptsAtTop', $slider);
	$at_end = true;
...
function atw_slider_do_gallery( $qargs, $slider, $ids = null) {
    ...
    if ( !$qargs || !empty( $ids ) ) {
        ...
        // get posts
        if ( !$qargs->have_posts()) {
            $content .= '<em>' . __('No posts found.','show-sliders') . ' ' . __('Slider Post slug: `', 'show-sliders') . $qargs->query['name'] .
                __('` - Slider: `', 'show-sliders') . $slider . '`.</em>';
